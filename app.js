...
function requestPanelConfirmation(message) {
...
function resolvePanelConfirmation(confirmed) {
...
async function deleteTemplates() {
  ...
  if (!(await requestPanelConfirmation("确定删除选中的 " + state.selectedTemplates.size + " 个模板吗？"))) return;
  try {
...
function bindEvents() {
  ...
  $("confirm-cancel").addEventListener("click", () => resolvePanelConfirmation(false));
  $("confirm-accept").addEventListener("click", () => resolvePanelConfirmation(true));
  $("confirm-dialog").addEventListener("click", (event) => {
    if (event.target === $("confirm-dialog")) resolvePanelConfirmation(false);
  });
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape") resolvePanelConfirmation(false);
  });
